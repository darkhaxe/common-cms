...
export const fetchImagesFromA2Q = async () => {
    console.log('开始执行文件抓取到七牛');
    let IMDbCharacters = require(jsonPath('IMDb.json'));
    ...
    IMDbCharacters = R.map(async item => {
        ...
        for (let i = 0; i < item.images.length; ++i) {
            ...
            try {
                ...
                console.log(`上传第${i}张图片:${item.name} url=${item.images[i]}`)
            } catch (e) {
    ...
    writeFileSync('./completedImgs.json', JSON.stringify(IMDbCharacters, null, 4), 'utf8')
